...
new class extends Component {
    ...
    public function register()
    {
        ...
        $userData['isCompany'] = $this->isCompany;
        ...
        @if($showEmailRegistration)
            <form wire:submit="register" class="space-y-5">
                ...
                <div class="flex items-center">
                    <input type="checkbox" wire:model="isCompany" id="checked-checkbox"
                           class="flex justify-center items-center w-5 h-5 rounded border border-gray-300 dark:border-gray-700 custom-checkbox text-neutral-900">
                    <label for="checked-checkbox" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Select
                        if company account</label>
                </div>
                ...
                @if($showEmailField)
                    ...
                    @endphp
                    <x-auth::elements.input :label="config('devdojo.auth.language.register.email_address')"
                                            id="email"
                ...
                @if($showPasswordField)
                    <x-auth::elements.input :label="config('devdojo.auth.language.register.password')"
                                            type="password"
                                            wire:model="password" id="password" name="password"
                                            data-auth="password-input" autocomplete="new-password" required/>
                @endif
                ...
                @if($showPasswordConfirmationField)
                    <x-auth::elements.input :label="config('devdojo.auth.language.register.password_confirmation')"
                                            ...
                                            data-auth="password-confirmation-input" autocomplete="new-password"
                                            required/>
                @endif
